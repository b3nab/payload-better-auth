...
export const processMultipart: ProcessMultipart = async ({
  options,
  request,
}) => {
  ...
  busboy.on('file', (field, file, info) => {
    ...
    const uploadTimer = createUploadTimer(options.uploadTimeout, () => {
      return abortAndDestroyFile(
        file,
        new APIError(
          `Upload timeout for ${field}->${filename}, bytes:${getFileSize()}`,
        ),
      )
    })
    ...
    file.on('limit', () => {
      ...
      if (options.abortOnLimit) {
        debugLog(
          options,
          `Upload file size limit reached ${field}->${filename}.`,
        )
        cleanup()
        abortAndDestroyFile(
          file,
          new APIError(
            options.responseOnLimit,
    ...
    file.on('end', () => {
      ...
      if (!name && size === 0) {
        ...
        return debugLog(
          options,
          `Don't add file instance if original name and size are empty`,
        )
      }
  ...
  busboy.on(
    ...
    (
      err = new APIError(
        'Busboy error parsing multipart request',
