...
2. **Better Auth endpoints** mounted at `/api/auth/*` by Better Auth itself. Everything related to sign-up, sign-in, 2FA, social providers, passkeys, sessions and so on lives here. The full reference is in the [Better Auth docs](https://www.better-auth.com/docs).
