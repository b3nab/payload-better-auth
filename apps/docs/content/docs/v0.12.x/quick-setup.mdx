...
```package-install
@b3nab/payload-better-auth better-auth better-auth-harmony
```

`better-auth` and `better-auth-harmony` are peer dependencies: install them alongside the plugin. The remaining peers (payload, next, react, `@payloadcms/next`, `@payloadcms/ui`) are already part of every Payload app - see [Installation](./installation) for the supported ranges.

## 2. Create the plugin config in its own file

Keeping the config separate from `payload.config.ts` keeps things readable and gives you good type inference. Wrap the object in `defineBetterAuthPluginOptions` so TypeScript validates the shape while keeping full type inference for your plugins, and add the `declare module` registration so `payload.betterAuth` is typed with your options everywhere. How the two pieces work is explained in [Auth Instance](./auth-layer/auth-instance).
...
Note the `/nextjs` subpath: since 0.12 the auth layer has its own entry point ([why](./migration-to-0.12)). `getAuth()` returns the Better Auth instance the plugin manages for you (`payload.betterAuth`). Thanks to the registration from step 2, any plugin you add under `betterAuth.plugins` (custom roles, 2FA, social providers, etc.) is reflected in the `auth.api.*` typings out of the box.
...
Inside Payload hooks, endpoints, or anywhere a `payload` instance is already in scope, skip the layer and use `payload.betterAuth` directly.
...
- [Exports](./api/exports) - package entry points (`.`, `/nextjs`, `/client`, `/rsc`)
- [Migrating to 0.12](./migration-to-0.12) - coming from 0.11.x
